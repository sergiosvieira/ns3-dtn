...
static Vector2d destination(1010.0, 10.0);
...
LinkBundle RTTrendOfDelivery::getBestLink(LinkBundleList lbl) {
        ...
        for (std::map<BundleEndpointId, TrendOfDeliveryTable>::iterator it =
                ...
                double tod = getFuzzy((*it).first);
                NS_LOG_DEBUG("(" << m_node->GetId() << ") it eid = " << (*it).first << " it tod = " << tod);
                if (tod > best_tod && (*it).second.m_expTime > Simulator::Now() ) {
        ...
        if (eid.GetId() == GetBundleEndpointId().GetId()) {
                NS_LOG_DEBUG("(" << m_node->GetId() << ") Do not copy!");
                return LinkBundle(0, 0);
        } else {
                NS_LOG_DEBUG("(" << m_node->GetId() << ") copy to " << eid);
...
void RTTrendOfDelivery::SendNeighHello(Ptr<Socket> socket, BundleEndpointId eid) {
        ...
        //NS_LOG_DEBUG("(" << m_node->GetId() << ") "<< mobilityModel->GetVelocity());
...
void RTTrendOfDelivery::printTable() {
        ...
        for (std::map<BundleEndpointId, TrendOfDeliveryTable>::iterator it = m_table.begin(); it != m_table.end(); ++it) {
                NS_LOG_DEBUG((*it).first << "\t" << (*it).second.m_expTime.GetSeconds() << "\t\t" << (*it).second.m_timeStamp.GetSeconds() << "\t\t" << (*it).second.m_pos << "\t" << (*it).second.m_vel << "\t\t" << getFuzzy((*it).first));
        }
        NS_LOG_DEBUG("");
}
...
double RTTrendOfDelivery::getFuzzy(BundleEndpointId eid) {
        double result = 0.0;
        ...
        m_tod.setFuzzy(my_pos_, destination, m_transmissionRange);

        Vector2d w = destination - my_pos_;
